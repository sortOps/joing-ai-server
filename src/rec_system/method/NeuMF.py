...
class NeuMF(torch.nn.Module):
    def __init__(self, config):
        ...
        self.embedding_item_category = torch.nn.Embedding(
            config['num_item_categories'], self.meta_latent_dim)
        self.embedding_media_type = torch.nn.Embedding(
            2, self.meta_latent_dim)  # 2: short, long
        self.embedding_channel_category = torch.nn.Embedding(
            config['num_channel_categories'], self.meta_latent_dim)
        self.embedding_subscribers = torch.nn.Embedding(
            config['max_subscribers'], self.meta_latent_dim)
    ...
    def load_pretrain_weights(self):
        ...
        self.affine_output.weight.data = 0.5 * \
            torch.cat([mlp_model.affine_output.weight.data,
